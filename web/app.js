...
function renderStandards(candidates) {
  ...
  candidates.forEach((candidate, index) => {
    ...
    score.textContent = `score ${Number(candidate.score).toFixed(3)}`;

    const copyBtn = document.createElement("button");
    copyBtn.type = "button";
    copyBtn.className = "copy-btn is-icon-only";
    copyBtn.setAttribute("aria-label", `Copy ${candidate.full_id}`);
    copyBtn.dataset.copyValue = candidate.full_id;
    copyBtn.innerHTML = `
    ...
    item.append(rank, body, score, copyBtn);
